...
function searchEventsByCountry() {
    ...
    const country = refs.searchCountryRef.options[refs.searchCountryRef.selectedIndex]
    ...
    countryEvents.fetchEventsByCoutry(countryCode)
        .then(events => {
            /* якщо результатів не знайдемо, виводимо нотифікацію */
            if (events.length === 0) { notificationError }
